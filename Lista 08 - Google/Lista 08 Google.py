...
# se a palavra começca com z,m,b e nao as contem na ultima letra
for i in range( 0, len(txtB) ):

    # preoposicoes
    if ( ( txtB[i][:1:] in letra_zumbi ) and not ( txtB[i][-1::] in letra_zumbi ) ):
        preposicoes.append( txtB[i])

    # verbos
    if ( ( len( txtB[i] ) == 7 ) and not ( txtB[i][-1::] in letra_zumbi ) ):
        verbos.append( txtB[i] )

    #verbos em primeira pessoa
    if ( ( len( txtB[i] ) == 7 ) and not ( txtB[i][:1:] in letra_zumbi ) and not ( txtB[i][-1::] in letra_zumbi ) ):
        verbos1p.append( txtB[i] )

# ORDENACOES

    # define a ordem de tras pra frente pq adiciona com append
    # z m b t s h j p n w l r c x k q v d g f
    alfabeto = list( 'zmbtshjpnwlrcxkqvdgf' )
    ordered = []

    # pra cada letra do alfabeto
    for i in range( 0, len( alfabeto) ):
        # adiciona propriamente as palavras
        for x in range ( 0, len( txtB ) ):
            if ( txtB[ x ][:1:] == alfabeto[i] ):
                ordered.append( txtB[ x ] )
...
# NUMEROS
    ...
    for i in range ( 0, len( txtB ) ):
        ...
        w = list( txtB[i] )
        n = sum( ( lambda x: [ alfabeto.index(x[i]) * 20 ** i for i in range( 0, len(x) ) ] ) ( w ) )

        for y in w:
            if not w.count(y) > 1 and not n%42:
                magicos.append( n )

# RESULTS

print( preposicoes, '\n\n')
print('No texo B existem', len( preposicoes ), 'preoposições Googlon. ')

print('\n\n'+10*'=='+'\n\n')

print( verbos, '\n\n')
print('No texo B existem', len( verbos ), 'verbos Googlon. ')

print('\n\n'+10*'=='+'\n\n')

print( verbos1p, '\n\n')
print('No texo B existem', len( verbos1p ), 'verbos Googlon !! em Primeira Pessoa !!. ')

print('\n\n'+10*'=='+'\n\n')
print('\n\nO texto B ordenado de acordo com o alfabeto Googlon (zmbtshjpnwlrcxkqvdgf): \n\n', ' '.join(ordered))

print('\n\n'+10*'=='+'\n\n')
print('\n\nO texto B possui', len( magicos ), ' números mágicos, sendo eles: \n\n', magicos )
